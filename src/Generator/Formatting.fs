...
module FSharp =
    module TypeToString =
        ...
        let rec private typeToString (typ: GLType) =
            match typ with
            | GLType.Void -> "unit"
            | GLType.Pointer(GLType.Void) -> "voidptr"
            | GLType.GLenum inner -> inner.groupName
            | GLType.Pointer inner -> "nativeptr<" + typeToString inner + ">"
            | GLType.GLint -> "int"
            | GLType.GLboolean -> "bool"
            | GLType.GLdouble -> "float"
            | GLType.GLbyte -> "sbyte"
            | GLType.GLfloat -> "float32"
            | GLType.GLchar -> "char"
            | GLType.GLcharARB -> "char"
            | GLType.GLclampf -> "float32"
            | GLType.GLfixed -> "int32"
            | GLType.GLint64 -> "int64"
            | GLType.GLint64EXT -> "int64"
            | GLType.GLintptr -> "System.IntPtr"
            | GLType.GLshort -> "int16"
            | GLType.GLsizei -> "uint32"
            | GLType.GLsizeiptr -> "System.UIntPtr"
            | GLType.GLubyte -> "byte"
            | GLType.GLuint -> "uint32"
            | GLType.GLuint64 -> "uint64"
            | GLType.GLuint64EXT -> "uint64"
            | GLType.GLushort -> "ushort"
            | GLType.GLvdpauSurfaceNV -> "System.IntPtr"
            | GLType.GLhalfNV -> "OpenToolkit.Mathematics.Half"
            | GLType.GLbitfield -> "uint32"
            | GLType.GLclampd -> "float"
            | GLType.GLclampx -> "int32"
            | GLType.GLeglClientBufferEXT -> "System.IntPtr"
            | GLType.GLeglImageOES -> "System.IntPtr"
            | GLType.GLhandleARB -> "uint32"
            | GLType.GLintptrARB -> "System.IntPtr"
            | GLType.GLsizeiptrARB -> "System.UIntPtr"
            | GLType.GLsync -> "System.IntPtr"
            | GLType.Struct_cl_context -> "Struct_cl_context"
            | GLType.Struct_cl_event -> "Struct_cl_event"
            | GLType.GLDEBUGPROC -> "DebugProc"
            | GLType.GLDEBUGPROCAMD -> "DEBUGPROCAMD"
            | GLType.GLDEBUGPROCARB -> "DEBUGPROCARB"
            | GLType.GLDEBUGPROCKHR -> "DEBUGPROCKHR"
            | GLType.RefPointer typ -> "byref<" + typeToString typ + ">"
    ...
    let generateDummyTypes =
        let usings = [ "System" ]
        let formatDummyType typ = sprintf "type %s = struct end" typ.name
        seq {
            yield "namespace " + dummyTypesNamespace |> writeLine
            for using in usings -> writeLine (sprintf "open %s" using)
            ...
            yield! enums
                   |> Array.Parallel.collect (fun enum ->
                       [| if enum.enumCases.Length > 0 then
                           yield sprintf "type %s =" enum.groupName
                                 |> writeLine
                           ...
                           for case in formattedCases -> writeLine case
                           yield unindent
                       |])
        }
        |> execute
                          ...
                          let additional =
                              if func.genericTypes.Length > 0 then
                                ...
                                let constraints =
                                    let cs =
                                        genericTypes
                                        |> Array.map (sprintf "%s : struct")
                                        |> String.concat " and "
                                    " when " + cs
                                "<" + inner + constraints + ">"
                              else ""
                          yield "abstract member " + func.prettyName + additional + " : "
                                + "(" + formattedParams + ")"  + "->" + retTypeAsString |> write
                          yield writeEmptyLine |])
            yield unindent
        }
        |> execute
    ...
    let generateStaticClass (functions: PrintReadyTypedFunctionDeclaration [])
        ...
        let _namespace, documentationFor = namespaceAndDocumentationFor details
        let usings =
            [ "System"; mathematicsNamespace; dummyTypesNamespace; advancedDlSupport ]
        seq {
            yield "namespace " + graphicsNamespace + "." + _namespace |> writeLine
            for using in usings -> sprintf "open %s" using |> writeLine
            ...
            yield """let instance = new NativeLibraryBuilder(ImplementationOptions.SuppressSecurity |
                                        ...
                                        ImplementationOptions.EnableOptimizations).ActivateInterface<IGL>(LibraryNameContainer.GetLibraryNameForCurrentPlatform())""" |> writeLine
            for func in functions do
                ...
                let formattedParams =
                    func.parameters
                    |> Array.Parallel.map
                        (fun p ->  "(" + p.prettyName + " : " + p.typ.prettyTypeName + ")")
                    |> String.concat " "
    ...
    let generateLibraryLoaderFor (_: GenerateDetails) = ""
